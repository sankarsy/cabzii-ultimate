...
import { blogs, testimonials } from "../data/travelData";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [cabsRes, pkgRes, drvRes] = await Promise.all([
        ...
        const [cabsJson, pkgJson, drvJson] = await Promise.all([cabsRes.json(), pkgRes.json(), drvRes.json()]);
        if (!cancelled) {
  ...
  const previewBlogs = useMemo(() => blogs.slice(0, 3), []);
  const previewTestimonials = useMemo(() => testimonials.slice(0, 3), []);
  ...
  const handleQuickSearch = () => {
    if (searchTab === "tours") {
      const q = quickForm.pickup.trim() || "tour package";
