...
/** Minimal home header — logo, search, login */
function EmtHomeHeader({ loggedIn, logout, menuOpen, setMenuOpen, linkClass }) {
  return (
    <>
...
export default function MmtHeader({ transparent = false, homeLayout = false }) {
  ...
  }, [transparent, pathname, homeLayout]);
  ...
  if (homeLayout) {
    return (
      <header
        className={`fixed left-0 right-0 top-0 z-[100] transition-transform duration-300 ease-out will-change-transform ${headerClass} ${
  ...
  return (
    ...
    >
      <div className="relative mx-auto flex min-h-14 w-full max-w-7xl items-center gap-2 px-3 py-1 sm:min-h-[4.25rem] sm:gap-3 sm:px-6">
        <div className="flex min-w-0 shrink-0 items-center gap-1 sm:gap-2">
          <Link href="/" className="min-w-0 py-0.5" aria-label={`${BRAND.fullName} home`}>
            <CabziiLogo onDark showDomain showTagline className="!text-base sm:!text-xl lg:!text-[1.55rem]" />
          </Link>
        ...
        <div className="pointer-events-none absolute left-1/2 top-1/2 hidden w-[min(100%,36rem)] -translate-x-1/2 -translate-y-1/2 px-4 lg:block">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          ...
          {loggedIn ? (
            <>
              <Link href="/account" className="hidden rounded-full px-3 py-2 text-sm font-medium text-white/90 hover:bg-white/10 hover:text-white lg:inline-block">
                Account
              </Link>
              <Link href="/my-bookings" className="hidden rounded-full px-3 py-2 text-sm font-medium text-white/90 hover:bg-white/10 hover:text-white lg:inline-block">
                My Trips
              </Link>
              <button type="button" onClick={logout} className="hidden rounded-full border border-white/25 px-3 py-2 text-sm font-semibold text-white hover:bg-white/10 lg:inline-flex">
                Logout
              </button>
            </>
          ) : (
            <Link href="/login" className="hidden rounded-full border border-white/30 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10 lg:inline-flex">
              Login
            </Link>
          )}
        </div>
      ...
      {menuOpen ? (
        <nav id="mmt-mobile-nav" className="border-t border-white/10 bg-cabzii-header px-4 py-4 shadow-lg lg:hidden" aria-label="Mobile">
          ...
          <div className="flex flex-col gap-0.5 sm:grid sm:grid-cols-2 sm:gap-1">
            {NAV_LINKS.map((item) => (
          ...
          <div className="mt-3 flex flex-wrap gap-2 border-t border-white/10 pt-3">
            {loggedIn ? (
              <>
                <Link href="/account" className={linkClass("/account")} onClick={() => setMenuOpen(false)}>Account</Link>
                <Link href="/my-bookings" className={linkClass("/my-bookings")} onClick={() => setMenuOpen(false)}>My Trips</Link>
                <button type="button" onClick={logout} className={linkClass("/account")}>Logout</button>
              </>
            ) : (
              <Link href="/login" className={linkClass("/login", true)} onClick={() => setMenuOpen(false)}>Login / Sign up</Link>
            )}
          </div>
        </nav>
      ) : null}
