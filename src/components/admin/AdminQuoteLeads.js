...
export default function AdminQuoteLeads({ token }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const q = new URLSearchParams({ admin: "1", limit: "50" });
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-sm font-bold text-slate-900">WhatsApp quote leads</h3>
          <p className="text-xs text-slate-500">Quote Ref in the WhatsApp message matches Lead ID below. Vendor view is limited to own vehicles.</p>
        </div>
      </div>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row._id} className="border-t border-slate-100">
                <td className="px-2 py-2 whitespace-nowrap">{row.createdAt ? new Date(row.createdAt).toLocaleDateString("en-IN") : "—"}</td>
                ...
                <td className="px-2 py-2">
                  <div>{row.ctaLocation || "—"}</div>
                  <div className="text-slate-400">{row.sourcePage || ""}</div>
                </td>
