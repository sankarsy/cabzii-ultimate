...
const VehicleSeoPanel = dynamic(() => import("./vehicles/VehicleSeoPanel"), {
  ssr: false,
  loading: () => (
...
export function AdminProductSeoSection({
  ...
  const panelForm = {
    ...
    images: Array.isArray(form.images)
      ? form.images
      : form.image
        ? [{ url: form.image, type: "cover", alt: form.imageAlt || "", title: form.imageTitle || "" }]
        : []
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <VehicleSeoPanel
        ...
        patch={(updates) => {
            const next = { ...updates };
            if (next.metaKeywords != null) next.seo = next.metaKeywords;
            if (next.vehicleName != null && titleField === "name") next.name = next.vehicleName;
            const enterpriseKeys = Object.keys(emptyEnterpriseSeoFields());
            const enterpriseSeo = { ...(form.enterpriseSeo || emptyEnterpriseSeoFields()) };
            enterpriseKeys.forEach((key) => {
              if (key in next) enterpriseSeo[key] = next[key];
            });
