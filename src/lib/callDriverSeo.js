...
export const DEFAULT_CALL_DRIVER_SEO = {
  local: {
    ...
    seoDescription:
      "Book a local Call Driver in Chennai for your own car. 3-hour city trips and a professional driver assigned after you confirm.",
...
<p>The usual city package starts at <strong>3 hours (₹450)</strong>. Extra hour is <strong>₹100</strong>. Night charge ₹100 from 10:15 pm to 5:30 am. Confirm the fare on the form above before you pay.</p>
...
<p>Need a Cabzii vehicle instead? Open <a href="/car-rental/chennai-city-cabs">cab booking Chennai</a> or the <a href="/tariff">Chennai tariff</a>. For a driver on a highway trip in your car, use <a href="/call-driver/book?service=outstation">outstation Call Driver</a>.</p>`,
    faqs: [
      ["How many hours is a local Call Driver booking?", "The local package starts at 3 hours (₹450). Extra hours are ₹100 as shown when you calculate the fare."],
...
<p>Return outstation is typically a <strong>12-hour day at ₹1,500</strong> plus driver accommodation. One-way is <strong>₹1,700</strong> (minimum 250 km, bus fare included). Extra hours on return trips are ₹100. Days count from travel date to return date (inclusive).</p>
...
<p>City-only hours belong on <a href="/call-driver/book?service=local">local Call Driver</a>. Airport pickup or drop in your car: <a href="/call-driver/book?service=airport">airport Call Driver</a>.</p>`,
    faqs: [
      ["Is food and hotel for the driver included?", "On return trips, accommodation for the driver is extra. One-way includes bus fare in the ₹1,700 rate."],
      ["Can I book a round trip in my own car?", "Yes. Choose Return, set travel and return dates — days are counted inclusive — then calculate the fare. Driver stay is extra. The same driver is assigned after confirmation."]
    ]
  },
  airport: {
...
<p>Use this when someone should collect your car from home, drive to the airport, or bring the car to arrivals. Minimum hours follow the airport Call Driver tariff (3 hours from ₹450, extra hour ₹100).</p>
...
<p>Minimum hours apply (5). Extra hours are ₹100. Per driver ₹600; one supervisor at ₹700 for every 10 drivers. Enter the event location in notes if it is not the pickup field.</p>
